...
const traceEventTypeInstant = 'I'.charCodeAt(0);
const traceEventTypeAsyncBegin = 'S'.charCodeAt(0);
const traceEventTypeAsyncEnd = 'F'.charCodeAt(0);
const traceEventTypeCounter = 'C'.charCodeAt(0);
...
Console.prototype.time = function time(name, id, category, args) {
  ...
  if (this.tracing.isEnabled(category)) {
    this.tracing._binding.emit(traceEventTypeAsyncBegin, name, id, category, args);
  }
};
...
Console.prototype.timeEnd = function timeEnd(name, id, category, args) {
  ...
  if (!category) category = this.tracing.options.defaultConsoleTracingCategory;
  if (this.tracing.isEnabled(category)) {
    this.tracing._binding.emit(traceEventTypeAsyncEnd, name, id, category, args);
  }
};
...
Console.prototype.timeStamp = function(name, category, args) {
  ...
  if (!category) category = this.tracing.options.defaultConsoleTracingCategory;
  if (this.tracing.isEnabled(category)) {
    this.tracing._binding.emitInstant(traceEventTypeInstant, name, undefined, category, args);
  }
};
...
Console.prototype.count = function(name, value, id, category) {
  ...
  if (!category) category = this.tracing.options.defaultConsoleTracingCategory;
  if (this.tracing.isEnabled(category)) {
    this.tracing._binding.emit(traceEventTypeCounter, name, id, category, value);
  }
};
...
Tracing.prototype.emit = function emit(category, tracingEvent) {
  ...
  if (Array.isArray(category)) {
    if (!category.find(function (c) { return this[c]; }, this._enabledCategories)) return;
  } else {
    if (!this._enabledCategories[category]) return;
  }
  ...
  switch (tracingEvent.eventType) {
    case 'begin':
      eventType = traceEventTypeAsyncBegin;
      args = tracingEvent.args;
      break;
    case 'end':
      eventType = traceEventTypeAsyncEnd;
      args = tracingEvent.args;
      break;
    case 'instant':
      eventType = traceEventTypeInstant;
      args = tracingEvent.args;
      break;
    case 'count':
      eventType = traceEventTypeCounter;
      args = tracingEvent.value;
      break;
    default: throw new TypeError('Trace event must include one of the ' +
